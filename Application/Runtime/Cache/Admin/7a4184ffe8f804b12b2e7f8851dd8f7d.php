	...
	<!-- <div class="sdiv">
		<form action="<?php echo U(MODULE_NAME.'/Article/cateSearch');?>" method="post" id="cateForm">
			...
			<select name="acid" id="slNcate">
				  <?php if(is_array($acate)): foreach($acate as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["html"]); echo ($v["cname"]); ?></option><?php endforeach; endif; ?>
			</select>
...
<script type="text/javascript">
	var dataUrl='<?php echo U("Article/lists");?>',aAdd='<?php echo U(MODULE_NAME."/Article/addArticle");?>',delUrl='<?php echo U(MODULE_NAME."/Article/remove");?>',editUrl='<?php echo U(MODULE_NAME."/Article/editArticle",'','');?>',deleleUrl='<?php echo U(MODULE_NAME."/Article/articleDelete",'','');?>',pRoot='/hzyj';
</script>
...
<script type="text/javascript">
  BUI.use(['common/search','common/page'],function (Search) {
    ...
    var columns = [{title: '标题', width:'40%',dataIndex: 'title'},
            {title : '文章类别',dataIndex :'cname'},
