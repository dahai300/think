...
<script type="text/javascript">
	var dataUrl='<?php echo U("Resumer/lists");?>',Resumer_add='<?php echo U(MODULE_NAME."/Resumer/add");?>',delUrl='<?php echo U(MODULE_NAME."/Resumer/remove");?>',editUrl='<?php echo U(MODULE_NAME."/Resumer/edit",'','');?>',deleleUrl='<?php echo U(MODULE_NAME."/Resumer/delete",'','');?>',pRoot='';
</script>
...
<script type="text/javascript">
  BUI.use(['common/search','common/page'],function (Search) {
    ...
    var columns = [{title: '编号',dataIndex: 'id',sortable: true},
            {title : '姓名',dataIndex :'name'},
            {title : '性别',dataIndex :'sex',renderer: BUI.Grid.Format.enumRenderer({"1":"男","2":"女"})},
            {title : '出生年月',dataIndex :'borndate',sortable: true,renderer : Grid.Format.dateRenderer},
            {title : '职位',dataIndex :'tobe'},
           // {title : '入库时间',dataIndex :'time',sortable: true,renderer : Grid.Format.dateRenderer},
            /*{title : '简历附件', width:'25%',dataIndex :'attachs',renderer:function(value,obj){
              var dlstr=pRoot+"/Data/Uploads/files/"+obj.attachs;
    ...
    var  search = new Search({
      ...
      function addFunction(){
        window.location.href=Resumer_add;
      }
